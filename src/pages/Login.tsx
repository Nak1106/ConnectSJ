...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError("");
    if (!calstateId || !password) {
      setError("Please enter both CalState ID and password.");
      return;
    }
    setIsLoading(true);
    try {
      const email = calstateId.includes("@")
        ? calstateId
        : `${calstateId}@yourdomain.edu`;
      ...
      localStorage.setItem("user", calstateId);
      onLogin();
      navigate("/dashboard");
    } catch (err: any) {
      if (err.code === "auth/user-not-found")
        setError(
          "No account found with that CalState ID. Please sign up first."
        );
        ...
        <motion.div variants={itemVariants}>
          <Card className="mb-4">
            <form onSubmit={handleSubmit}>
              ...
              <Input
                label={t("auth.calstateId")}
                id="calstateId"
                type="text"
                value={calstateId}
                onChange={(e) => setCalstateId(e.target.value)}
                icon={<User size={18} />}
